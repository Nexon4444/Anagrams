...
//  test("wordAnagrams cats (2pts)") {
//    assertEquals(wordAnagrams("cats").toSet, Set("scat", "admirer"))
//  }
  ...
  test("subtract: bob - bob (10pts)") {
    val lard = List(('b', 2), ('o', 1))
    val r = List(('b', 2), ('o', 1))
    val lad = List()
    assertEquals(subtract(lard, r), lad)
  }
  ...
  test("sentence anagrams: Bob") {
    val sentence = List("Bob")
    val anas = List(
      List("bob"),
